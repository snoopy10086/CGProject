...
void InitialThings() {
	...
	Sphere* b1 = new Sphere(3.6, 0.2, 4.6);//放到传送带上4.70.25.2
	printf("B1 HAS A TYPE %d\n", b1->RetType());
	YueBing* c1 = new YueBing(4.6, 0.2, 4.6, b1->RetType());//放到传送带上4.70.25.2
	printf("C1 HAS A TYPE %d\n", c1->RetType());
	Sphere* b2 = new Sphere(3.4, 0.2, 4.6,1);//放到传送带上4.70.25.2
	printf("B2 HAS A TYPE %d\n", b2->RetType());
	YueBing* c2 = new YueBing(4.4, 0.2, 4.6, b2->RetType());//用这个rettype函数就可以创建馅料类型对应的月饼类型
	printf("C2 HAS A TYPE %d\n", c2->RetType());
	Sphere* b3 = new Sphere(3.2, 0.2, 4.6, 2);//放到传送带上4.70.25.2
